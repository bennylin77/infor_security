...
#================================================================================================================================for menu  
  ...
  def unShowing
    ...
    adm_user=AdmUser.find(session[:adm_user_id])
    if adm_user.permission_config.job & Infor::Application.config.READ_PERMISSION != 0 
      @jobs = Job.paginate(:per_page => 50, :page => params[:page], :conditions => ["( ( (stage1 = 'un' or stage1 = 'returned') and stage5 = 'un' and deleted = ? and always_handle = ?) and ( (isflood_scan=1 and log_count>50) or (isflood_scan=0 and alert <> ? and log_count>20) or (log_count >= 1000 and alert <> ?) or PA = ? ) )", false, true, true, false, false], :joins => :job_detail).order('id DESC')
    else
...
def test
	...
	@jds = JobDetail.joins(:job).where('job_details.updated_at BETWEEN ? AND ? AND src_ip REGEXP "^140\.113\.*"',@time.strftime('%F 00:00:00'), @time.strftime('%F 23:59:59')).order('job_details.last_count DESC')
