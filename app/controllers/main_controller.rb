...
#================================================================================================================================for comment  
  def comment 
    if request.post?               
      ...
      @comment.save!
      adm_user=AdmUser.find_by_name('林起')
      SystemMailer.sendComment(adm_user, @comment).deliver
      adm_user=AdmUser.find_by_name('葉仲軒')
      SystemMailer.sendComment(adm_user, @comment).deliver
      adm_user=AdmUser.find_by_name('許郡泓')
      SystemMailer.sendComment(adm_user, @comment).deliver                   
      redirect_to :controller=>'main', :action=>'index', :notice=>'已將您寶貴的意見送出'
    else
      @comment=Comment.new
    end     
  end
