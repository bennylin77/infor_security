...
class CommentListController < ApplicationController
	...
	def CommentDetailShow
		...
		if request.post?
			if params[:assigned_user].nil? and params[:result].nil?
				@comment.handling_adm_user_id =params[:assigned_user_change]
				@comment.save!
			...
			elsif params[:assigned_user_change].nil? and params[:result].nil?
